...
OWNER_ID = 631833072  # ТОЛЬКО ТЫ МОЖЕШЬ АКТИВИРОВАТЬ БОТА
...
print(f"👑 Владелец: {OWNER_ID} (только ты активируешь бота)")
...
print(f"👑 Активировать бота может только владелец: {OWNER_ID}")
print("=" * 50)
...
for event in longpoll.listen():
    if event.type == VkEventType.MESSAGE_NEW:
        ...
        if cid:
            ...
            # ПРОВЕРКА АКТИВАЦИИ БОТА - ТОЛЬКО ВЛАДЕЛЕЦ МОЖЕТ АКТИВИРОВАТЬ
            ...
            if not activated or activated[0] == 0:
                if msg == '/start' and uid == OWNER_ID:
                    ...
                    if uid == OWNER_ID:
                        send(peer, "❌ Бот не активирован! Напиши /start чтобы активировать")
                    continue
        ...
        # 👤 КОМАНДЫ ДЛЯ ВСЕХ
        if msg == '/help' or msg == 'помощь':
            txt = """📚 **BLACK FIB BOT - СПРАВКА**
━━━━━━━━━━━━━━━━━━━━
...
• /pull название - создать привязку
• /pullinfo - инфо о привязке
• /delpull - удалить привязку
• /srnick @user - сброс ника везде
• /ssetnick @user ник - установить ник везде
• /srrole @user - сброс роли везде
• /srole @user роль - выдать роль везде
• /szov текст - супер упоминание

👤 **Владелец беседы:**
• /gremoverole @user - сброс ролей
• /news текст - новости во все чаты
• /addzam @user - зам.создателя
...
👑 **Создатель бота:**
• /adddev @user - дать права создателя
• /addzam @user - дать права зам.создателя
...
• /sync - синхронизировать БД"""
        ...
        # 🔴 СПЕЦ АДМИНИСТРАТОРЫ
            ...
            send(peer, "✅ /gban @user - глобальный бан")
        ...
        # 🟠 ЗАМ.СПЕЦ АДМИНА
            ...
            if not check_perm(uid, cid, 'admin'):
                ...
                if name:
                    ...
                    conn.commit()
                    send(peer, f"✅ Привязка '{name}' создана для этого чата")
                else:
            ...
            if not check_perm(uid, cid, 'admin'):
                ...
                if target:
                    ...
                    if len(parts) > 2:
                        ...
                        conn.commit()
                        send(peer, f"✅ {get_name(target)} | НИК '{nick}' УСТАНОВЛЕН ВЕЗДЕ")
                    else:
                ...
                if target:
                    ...
                    if len(parts) > 2:
                        ...
                        if role in ['helper', 'moderator', 'admin', 'owner']:
                            c.execute("UPDATE users SET role=? WHERE user_id=?", (role, target))
                            conn.commit()
                            send(peer, f"✅ {get_name(target)} | РОЛЬ '{role}' ВЫДАНА ВЕЗДЕ")
                        else:
                            send(peer, "❌ Роли: helper, moderator, admin, owner")
                    else:
            ...
            if not check_perm(uid, cid, 'admin'):
                ...
                for chat in get_all_chats():
                    try:
                        send(2000000000 + chat, f"📢 СУПЕР УПОМИНАНИЕ ОТ {get_name(uid)}:\n{text}")
                    except:
                        pass
                send(peer, "✅ Сообщение отправлено во все чаты")
        ...
        # АКТИВАЦИЯ БОТА В БЕСЕДЕ (ТОЛЬКО ВЛАДЕЛЕЦ)
